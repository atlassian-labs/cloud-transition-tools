...
class JCMAMigrationMappingLoader(
    ...
    private fun downloadIDMappings(
        ...
        try {
                ...
                when {
                    response.isError() -> {
                        ...
                        LoaderStatus(
                            ...
                            "Failed to download ID mappings. : ${response.status()} Error: ${response.text()}",
                        )
                    }
                    response.isAccepted() -> {
                        logger.info { "ID mappings are being generated" }
                        LoaderStatus(LoaderStatusCode.LOADING, "ID mappings are being generated by the server")
                    }
                    response.contentType()?.startsWith("text/csv") != true -> {
                        logger.error {
                            "Invalid response from server. Expected CSV. Obtained ${response.contentType()}"
                        }
                        LoaderStatus(LoaderStatusCode.FAILED, "Invalid response from server. Expected CSV")
                    }
                    else -> {
                        // get the csv stream and parse it
                        val csvStream: java.io.InputStream =
                            response.byteStream()
                                ?: return LoaderStatus(LoaderStatusCode.FAILED, "Failed to get CSV stream")
            ...
            status = LoaderStatus(LoaderStatusCode.FAILED, e.toString())
        }
